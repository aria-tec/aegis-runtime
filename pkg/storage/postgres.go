...
func (s *PostgresStore) CreateWorkflow(ctx context.Context, wf *domain.Workflow) error {
	now := time.Now().UTC()
...
func (s *PostgresStore) UpdateWorkflow(ctx context.Context, wf *domain.Workflow) error {
	wf.UpdatedAt = time.Now().UTC()
	query := `UPDATE workflows SET status = $1, current_step = $2, total_tokens_used = $3, max_steps = $4, token_budget = $5, result = $6, error_message = $7, updated_at = $8 WHERE id = $9`
...
func (s *PostgresStore) GetWorkflow(ctx context.Context, id string) (*domain.Workflow, error) {
	query := `SELECT id, goal, status, current_step, total_tokens_used, max_steps, token_budget, COALESCE(result, ''), COALESCE(error_message, ''), created_at, updated_at FROM workflows WHERE id = $1`
...
func (s *PostgresStore) AppendEvent(ctx context.Context, evt *domain.WorkflowEvent) error {
...
func (s *PostgresStore) GetEvents(ctx context.Context, workflowID string) ([]domain.WorkflowEvent, error) {
	query := `SELECT id, workflow_id, step_number, event_type, payload_json, tokens_used, duration_ms, created_at FROM workflow_events WHERE workflow_id = $1 ORDER BY step_number ASC, created_at ASC`
